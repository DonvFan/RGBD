...
from pf.dataset.base import BaseDataset, create_segmentation_data_sem, InconsistentBaseDataset, InconsistentSingleBaseDataset
from pf.dataset.panopli import PanopLiDataset, InconsistentPanopLiDataset, InconsistentPanopLiSingleDataset, create_segmentation_data_panopli, SegmentPanopLiDataset
    ...
    if config.dataset_class == "panopli":
        ...
        if not load_only_val:
            train_set = PanopLiDataset(Path(config.dataset_root), "train", (config.image_dim[0], config.image_dim[1]), config.max_depth, overfit=config.overfit, semantics_dir='m2f_semantics',
                                       ...
                                       create_seg_data_func=create_segmentation_data_panopli, subsample_frames=config.subsample_frames, load_depth=True, depth_scale=config.depth_scale)
