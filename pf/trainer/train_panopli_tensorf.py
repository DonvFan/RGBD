...
from pf.model.radiance_field.tensoRF import TensorVMSplit
from pf.model.renderer.panopli_tensoRF_renderer import TensoRFRenderer
...
class TensoRFTrainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # self.output_dir_result_testm.mkdir(exist_ok=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config.lambda_rgb > 0:
            ...
            loss_depth = self.loss(_output_depth, depth)
            ...
            loss = self.config.lambda_rgb * (loss_rgb + loss_tv + loss_dist_reg * self.current_lambda_dist_reg + loss_feat * self.config.lambda_feat) + loss_depth
            ...
            self.log("train/depth_loss", loss_depth, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
    ...
    def validation_epoch_end(self, val_step_outputs):
        ...
        with open(self.output_dir_result_testm, 'a+') as f:
            f.write(str('*'*5 + f"{self.global_step:06d}" + '*' * 5 + '\n'))
            f.write(result)
            f.write('\n\n')
