...
from pl.util.distinct_colors import DistinctColors
from pl.util.misc import visualize_depth, probability_to_normalized_entropy, get_boundary_mask
from pl.util.warmup_scheduler import GradualWarmupScheduler
from pytorch_lightning import seed_everything, Trainer
from pytorch_lightning.callbacks import ModelCheckpoint
from pytorch_lightning.loggers import TensorBoardLogger, WandbLogger

from pl.util.filesystem_logger import FilesystemLogger
...
def visualize_panoptic_outputs(p_rgb, p_semantics, p_instances, p_depth, rgb, semantics, instances, H, W, thing_classes, depth_gt = None,  visualize_entropy=True):
    alpha = 0.65
    ...
    if rgb is not None and semantics is not None and instances is not None:
        ...
        depth_img =  torch.zeros_like(img_gt)
        if depth_gt is not None:
            depth_img = visualize_depth(depth_gt.view(H ,W))
        ...
        stack = torch.cat([torch.stack([img_gt, img_semantics_gt, img_instances_gt, depth_img, torch.zeros_like(img_gt)]), torch.stack([img, img_semantics, img_instances, depth, img_sem_entropy])], dim=0)
    else:
