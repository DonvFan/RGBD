...
# import sys 
# sys.path.append('./')
from pf.dataset import get_dataset, get_inconsistent_single_dataset, get_segment_dataset
from pf.model.loss.loss import TVLoss, get_semantic_weights, SCELoss
from pf.model.radiance_field.tensoRF import TSDFVol
from pf.model.renderer.panopli_tsdfRF_renderer import TsdfRFRenderer
from pf.util.distinct_colors import DistinctColors
from pf.trainer import create_trainer, get_optimizer_and_scheduler, visualize_panoptic_outputs
from pf.util.metrics import psnr, ConfusionMatrix
from pf.util.panoptic_quality import panoptic_quality
...
class TensoRFTrainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.model = TSDFVol(feat_grid_dim = self.config.feat_grid_dim, num_semantic_classes=total_classes, dim_feature_instance=self.config.max_instances,output_mlp_semantics=output_mlp_semantics, use_semantic_mlp=self.config.use_mlp_for_semantics,use_feature_reg=self.config.use_feature_regularization,densitynet_weight_path = self.config.densitynet_weight_path)
        # grid_weight_path=self.config.grid_weight_path)

        self.renderer = TsdfRFRenderer(self.train_set.scene_bounds, [config.min_grid_dim, config.min_grid_dim, config.min_grid_dim],
        ...
        self.output_dir_result_testm = Path(f'runs/{self.config.experiment}/test_metric.txt')
        self.automatic_optimization = False
        self.distinct_colors = DistinctColors()

    def configure_optimizers(self):
        params = self.model.get_optimizable_parameters(self.config.lr * 1e2 , self.config.lr, weight_decay=self.config.weight_decay)
    ...
    def training_step(self, batch, batch_idx):
        ...
        depths = batch[0]['depths']
        output_rgb, output_semantics, output_instances, _output_depth, output_feats, depth_var = self(rays, True)
        output_rgb[~masks, :] = 0
        rgbs[~masks, :] = 0
        confs[~masks] = 0
        ...
        if self.config.lambda_rgb > 0:
            ...
            loss_feat = torch.zeros_like(loss_rgb)
            # loss_depth = torch.mean(depth_var * (depths - _output_depth)**2)
            loss_depth = self.loss(_output_depth[ind], depths[ind])
            ...
            loss = self.config.lambda_rgb * (loss_rgb +  loss_feat ) + loss_depth
            self.log("train/loss_rgb", loss_rgb, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
            self.log("train/loss_depth", loss_depth, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
    ...
    def validation_epoch_end(self, val_step_outputs):
        ...
        print(result)
        with open(self.output_dir_result_testm, 'a+') as f:
            f.write('*'* 5 + str(self.global_step)+ '*'*5+'\n')
            f.write(result)
            f.write('\n\n')
        ...
        for batch_idx, batch in enumerate(self.val_dataloader()):
            if batch_idx in self.config.visualized_indices:
                ...
                depth = batch['depth'].squeeze().to(self.device)
                ...
                rgbs[torch.logical_not(mask), :] = 0
                stack = visualize_panoptic_outputs(output_rgb, output_semantics, output_instances, output_depth, rgbs, rs_semantics, rs_instances, H, W, depth_gt = depth, thing_classes=self.train_set.segmentation_data.fg_classes)
    ...
    def train_dataloader(self):
        loaders = {
            0: DataLoader(self.train_set, self.config.batch_size, shuffle=True, pin_memory=False, drop_last=True, num_workers=self.config.num_workers)
        }
        ...
        loaders[1] = DataLoader(train_instance_set, self.config.batch_size_contrastive, shuffle=True, drop_last=True, collate_fn=train_instance_set.collate_fn, num_workers=self.config.num_workers)
        if self.config.segment_grouping_mode != "none":
            loaders[2] = DataLoader(self.train_segment_set, self.config.batch_size_segments, shuffle=False, drop_last=True, collate_fn=self.train_segment_set.collate_fn, num_workers=self.config.num_workers)
        return loaders

    def val_dataloader(self):
        return DataLoader(self.val_set, batch_size=1, shuffle=False, drop_last=False, num_workers=self.config.num_workers)
...
# @hydra.main(config_path='../config', config_name='m3d_s2', version_base='1.2')
# def main(config):
