...
from pl.dataset.base import create_segmentation_data_base, BaseDataset, process_bounding_box_dict
from preprocess.pl.preprocess_scannet import get_thing_semantics
from pl.util.camera import compute_world2normscene
from pl.util.misc import EasyDict
from pl.util.ray import get_ray_directions_with_intrinsics, get_rays, rays_intersect_sphere
...
class PanopLiDataset(BaseDataset):
    ...
    def load_sample(self, sample_index):
        ...
        ind = (fars <= 1e-5)
        if ind.any().cpu().item():
            default = (fars[ind].max()  + fars[ind].min())/ 2. 
