...
from plm.dataset import get_dataset, get_inconsistent_single_dataset, get_segment_dataset
from plm.model.loss.loss import TVLoss, get_semantic_weights, SCELoss
from plm.model.radiance_field.tsdfRF import TSDFVol
from plm.model.renderer.panopli_tsdfRF_renderer import TsdfRFRenderer
from plm.util.distinct_colors import DistinctColors
from plm.trainer import create_trainer, get_optimizer_and_scheduler, visualize_panoptic_outputs
from plm.util.metrics import psnr, ConfusionMatrix
from plm.util.panoptic_quality import panoptic_quality
...
class TensoRFTrainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.renderer = TsdfRFRenderer(self.train_set.scene_bounds, [config.min_grid_dim, config.min_grid_dim, config.min_grid_dim],
                                        semantic_weight_mode=self.config.semantic_weight_mode, stop_semantic_grad=config.stop_semantic_grad, n_samples=config.samples_num)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.config.lambda_rgb > 0:
            ...
            loss_depth = self.config.lambda_depth * self.loss(_output_depth[ind], depths[ind])
