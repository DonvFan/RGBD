...
from plm.util.transforms import trs_comp, dot
...
def compute_world2normscene(dims, intrinsics, cam2worlds, max_depth, rescale_factor=1.0):
    ...
    bounds = frustum_world_bounds(dims, intrinsics, cam2worlds, max_depth)
    center = (bounds[1] + bounds[0]) / 2.
    scale = torch.max(bounds[1] - bounds[0]) / 2.
    world2nscene = trs_comp(-center / (rescale_factor * scale), torch.eye(3), 1 / (rescale_factor * scale))
    return world2nscene
