...
from plm.dataset.base import create_segmentation_data_base, BaseDataset, process_bounding_box_dict
from plm.dataset.preprocessing.preprocess_scannet import get_thing_semantics
from plm.util.camera import compute_world2normscene
from plm.util.misc import EasyDict
from plm.util.ray import get_ray_directions_with_intrinsics, get_rays, rays_intersect_sphere


class PanopLiDataset(BaseDataset):

    def __init__(self, root_dir, split, image_dim, max_depth, overfit=False, num_val_samples=20, load_depth=False, load_feat=False, instance_dir='filtered_instance', semantics_dir="filtered_semantics",
                 instance_to_semantic_key='instance_to_semantic', create_seg_data_func=create_segmentation_data_base, subsample_frames=1, sc_classes='super', depth_scale = 1000., depth_z2d = False):
        ...
        self.is_thing = get_thing_semantics(sc_classes)
    ...
    def setup_data(self):
        ...
        self.all_pose_names = sorted([x.stem for x in (self.root_dir / "pose").iterdir() if x.name.endswith('.txt')], key=lambda y: int(y) if y.isnumeric() else y)
        ...
        if self.overfit:
            ...
            if (self.root_dir / "splits.json").exists():
                ...
                self.val_indices = np.arange(0, len(self.all_frame_names), len(self.all_frame_names) // self.num_val_samples)
        ...
        for sample_index in sample_indices:
            ...
            cam2world = np.array([[float(y.strip()) for y in x.strip().split()] for x in Path(self.root_dir / "pose" / f"{self.all_pose_names[sample_index]}.txt").read_text().splitlines() if x != ''])
        ...
        self.normal_max_depth = self.max_depth * self.normscene_scale
    ...
    def load_sample(self, sample_index):
        ...
        if self.load_depth:
            # print(self.root_dir)
            raw_depth = np.array(Image.open(self.root_dir / "depth" / f"{self.all_frame_names[sample_index].replace('_i', '_d')}.png"))
            
            raw_depth = raw_depth.astype(np.float32) / self.depth_scale
        ...
        if self.depth_z2d:
            temp_d = directions.flatten(end_dim = -2)
            temp_d = temp_d / temp_d.norm(dim = -1, keepdim=True)
            depth = depth[..., None] / temp_d[..., -1:] * temp_d
            depth = torch.norm(depth, dim = -1)

        # sphere_intersection_displacement = rays_intersect_sphere(rays_o, rays_d, r=1)  # fg is in unit sphere
        # sphere_intersection_displacement = rays_intersect_sphere(rays_o, rays_d, r = np.sqrt(3) + 1e-5)
        ...
        if self.load_depth:
            ...
            if ind.any().cpu().item():
                default = fars.max()
                fars = torch.where(fars <= 1e-5, torch.full_like(fars, default), fars)
            rays = torch.cat(
                [rays_o, rays_d, 0.005 *
                torch.ones_like(rays_o[:, :1]), fars], 1,
            )
        else:
            rays = torch.cat(
                [rays_o, rays_d, 0.005 *
                torch.ones_like(rays_o[:, :1]), torch.ones_like(rays_o[:, :1])], 1,
            )
...
class InconsistentPanopLiSingleDataset(PanopLiDataset):
    ...
    def __init__(self, root_dir, split, image_dim, max_depth, overfit=False, num_val_samples=8, max_rays=512, semantics_dir='filtered_semantics',
                 instance_dir='filtered_instance_inc', instance_to_semantic_key='instance_to_semantic_inc', create_seg_data_func=create_segmentation_data_base, subsample_frames=1, depth_scale = 4000., depth_z2d = True):
        super().__init__(root_dir, split, image_dim, max_depth, overfit, num_val_samples, semantics_dir=semantics_dir,
                         instance_dir=instance_dir, instance_to_semantic_key=instance_to_semantic_key, create_seg_data_func=create_seg_data_func, subsample_frames=subsample_frames, depth_scale=depth_scale, depth_z2d=depth_z2d)
        print('Preparing InconsistentPanopLiDataset...')
...
class SegmentPanopLiDataset(PanopLiDataset):
    ...
    def __init__(self, root_dir, split, image_dim, max_depth, overfit=False, num_val_samples=8, max_rays=512, semantics_dir='filtered_semantics',
                 instance_dir='filtered_instance_inc', instance_to_semantic_key='instance_to_semantic_inc', create_seg_data_func=create_segmentation_data_base, subsample_frames=1, depth_scale = 4000., depth_z2d = True):
        super().__init__(root_dir, split, image_dim, max_depth, overfit, num_val_samples, semantics_dir=semantics_dir,
                         instance_dir=instance_dir, instance_to_semantic_key=instance_to_semantic_key, create_seg_data_func=create_seg_data_func, subsample_frames=subsample_frames, load_depth=True, depth_scale=depth_scale, depth_z2d=depth_z2d)
